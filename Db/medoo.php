...
class medoo
{
    ...
    public function reConnect()
    {
        try {
            ...
            $this->pdo = new PDO('', $this->username, $this->password, $this->option);
            foreach ($commands as $value) {
                $this->pdo->exec($value);
            }
        } catch (PDOException$e) {
    ...
    public function query($query)
    {
        ...
        $log = date('Y-m-d H:i:s', time()) . $this->last_query();
        application_log('sql', $log);
        //mysql报错后重连
        if ($result === FALSE) {
            $err = $this->pdo->errorInfo();
            $log = date('Y-m-d H:i:s', time()) . $err[1] . $err[2] . $this->last_query();
            application_log('sql_error', $log);
